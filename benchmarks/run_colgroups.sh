...
# ChunkDB Column Groups Parametric Benchmark Runner
# Tests with 128 columns in 4 column groups
# Usage: ./run_colgroups.sh [full|quick]
#
# NOTE: This script requires the parametric_benchmark_colgroups example which
# is not currently in the repository. It was designed for a column-groups variant
# of the parametric benchmark. You can create it by copying parametric_benchmark.rs
# and adding column group configuration with 128 columns in 4 groups.
...
echo -e "${BLUE}================================================${NC}"
echo -e "${BLUE}ChunkDB Column Groups Parametric Benchmark${NC}"
echo -e "${BLUE}================================================${NC}\n"

# Check that the example exists
if [ ! -f "examples/parametric_benchmark_colgroups.rs" ]; then
    echo -e "${RED}Error: examples/parametric_benchmark_colgroups.rs not found${NC}"
    echo -e "${YELLOW}This benchmark requires the column-groups variant of the parametric benchmark.${NC}"
    echo -e "${YELLOW}See benchmarks/README.md for details.${NC}"
    exit 1
fi

# Create output directory with timestamp
TIMESTAMP=$(date +%Y%m%d_%H%M%S)
OUTPUT_DIR="${SCRIPT_DIR}/benchmark_colgroups_${TIMESTAMP}"
mkdir -p "$OUTPUT_DIR"
...
# Check mode
if [ "$MODE" == "quick" ]; then
    echo -e "${YELLOW}QUICK MODE: 27 configurations (~15 minutes)${NC}\n"
    CONFIGS=27
    ESTIMATED_TIME="15 minutes"

    # Backup original file
    cp examples/parametric_benchmark_colgroups.rs examples/parametric_benchmark_colgroups.rs.backup

    # Modify for quick run
    sed -i 's/let chunk_rows_range: Vec<u64> = (1..=10).map(|i| i \* 5000).collect();/let chunk_rows_range: Vec<u64> = vec![5000, 25000, 50000];/' examples/parametric_benchmark_colgroups.rs
    sed -i 's/let hash_buckets_range: Vec<u64> = (1..=10).map(|i| i \* 10).collect();/let hash_buckets_range: Vec<u64> = vec![10, 50, 100];/' examples/parametric_benchmark_colgroups.rs
    sed -i 's/let range_dim_range: Vec<u64> = (1..=10).map(|i| i \* 5000).collect();/let range_dim_range: Vec<u64> = vec![5000, 25000, 50000];/' examples/parametric_benchmark_colgroups.rs

    trap cleanup EXIT
    cleanup() {
        echo -e "\n${YELLOW}Restoring original file...${NC}"
        mv examples/parametric_benchmark_colgroups.rs.backup examples/parametric_benchmark_colgroups.rs
    }
elif [ "$MODE" == "full" ]; then
    echo -e "${RED}FULL MODE: 1000 configurations (~6-12 hours)${NC}\n"
    CONFIGS=1000
...
echo -e "  Columns:     ${GREEN}128 data columns${NC}"
echo -e "  Col Groups:  ${GREEN}4 groups (40 cols each)${NC}"
echo -e "  Queries:     ${GREEN}3 (with 3, 5, 7 columns)${NC}"
...
echo -e "${BLUE}[3/5]${NC} Compiling column groups benchmark..."
cargo build --example parametric_benchmark_colgroups --release 2>&1 | grep -E "(Compiling|Finished|error)" || true
...
echo -e "${YELLOW}Note: With 128 columns, this will be slower than standard benchmark${NC}"
...
cargo run --example parametric_benchmark_colgroups --release 2>&1 | tee "${OUTPUT_DIR}/benchmark_run.log"
...
echo -e "${YELLOW}Note: Plots will show column projection performance${NC}\n"
...
ChunkDB Column Groups Benchmark Results
========================================
...
- 4 column groups (~40 columns each)
- 3 queries testing column projection:
  * Q1: 3 columns with sensor filter
  * Q2: 5 columns with time range filter
  * Q3: 7 columns with combined filters
...
Plot Files:
  ...
  benchmark_3d_surfaces.png         - 3D visualizations
  benchmark_parameter_effects.png   - Parameter impact
  benchmark_best_configs.png        - Best configurations
...
2. Compare with standard benchmark to see column projection impact
...
echo -e "  ${GREEN}+${NC} benchmark_analysis.txt"
...
if [ -f "${OUTPUT_DIR}/benchmark_summary.txt" ]; then
    head -30 "${OUTPUT_DIR}/benchmark_summary.txt"
fi
